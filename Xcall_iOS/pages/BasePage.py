...
class Base(object):
    ...
    def click_wait_button_byclass(test_evm, btn_id):
        ...
        while i < time_interval_times:
            ...
            bt.click()
            break
# xcall 找到元素不能点击，可以采用如下定位方法
    def click_wait_button_byclass_1(test_evm, btn_id):
        i = 0
        time_interval = Config().get('time_interval')
        time_interval_times = Config().get('time_interval_times')
        while i < time_interval_times:
            ...
            if i == 500:
                raise ValueError("can't find elements")
            try:
                bt = test_evm.driver.find_elements_by_class_name(btn_id)
            except Exception:
                sleep(time_interval)
                continue
            bt[-1].click()
    ...
    def click_send_byname(test_evm, btn_id, keys):
        i = 0
        time_interval = Config().get('time_interval')
        time_interval_times = Config().get('time_interval_times')
        while i < time_interval_times:
            ...
            if i == 500:
                raise ValueError("can't find elements")
            try:
                bt = test_evm.driver.find_element_by_name(btn_id)
            except Exception:
                sleep(time_interval)
                continue
            bt.send_keys(keys)
